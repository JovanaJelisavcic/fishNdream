<template>
  <div class="content">
    ...
    >
      ...
      <template v-slot:cell(beginning)="{ item }">
        <b-row class="m-0 p-2">
          {{ moment(item.beginning).format("YYYY-MM-DD") }}
        </b-row>
      </template>
      <template v-slot:cell(ending)="{ item }">
        <b-row class="m-0 p-2">
          {{ moment(item.ending).format("YYYY-MM-DD") }}
        </b-row>
      </template>
...
import {
  getAllFutureCottageReservations,
  cancelResCottage,
  cancelActionResCottage,
} from "../../../../api";
export default {
  data() {
    return {
  ...
  methods: {
    async fetchAllReservations() {
      await getAllFutureCottageReservations()
        .then((response) => {
    ...
    async cancel(reservationId, actionRes) {
      if (actionRes) {
        await cancelActionResCottage(reservationId)
          .then(() => {
        ...
        await cancelResCottage(reservationId)
          .then(() => {
            alert("Canceled successfully");
            this.fetchAllReservations();
          })
